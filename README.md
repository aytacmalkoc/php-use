...
```bash
git clone https://github.com/kullaniciadi/php-switcher.git
cd php-switcher
```

2. Open `php-use.bat` and edit the paths to your PHP versions:
```batch
...
3. On first use, run `php-use.bat` to add the directory to PATH:
```bash
php-use.bat 8.4
```
...
- The `php-switcher` folder will be automatically added to PATH when you first run `php-use.bat`
- After opening a new terminal, you can use: `php-use 8.4` (without `.bat` extension if you have file extensions enabled)

**Git Bash:**
Add this line to your `~/.bashrc` or `~/.bash_profile`:
```bash
alias php-use='/c/path/to/php-switcher/php-use.bat'
```
...
3. On first run, the `php-switcher` folder is automatically added to PATH.

## Setting Your Own PHP Paths

Open `php-use.bat` and edit the `SET PHP_XX=` lines according to your PHP installation paths:
...
- To manually add to PATH: `set PATH=%PATH%;C:\php-switcher`

### Not working in Git Bash

- Make sure the `php` file is executable: `chmod +x php`
